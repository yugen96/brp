...
for i, [cscape_jk, dscape_jk] in enumerate(zip(cscape_ijk,dscape_ijk)):
    ...
    # Load the c- and d-scapes   
        ...
        cscape_med = np.load(npsavedir+"/cdscapes/cscape_NGC+WDmed.npy")
        dscape_med = np.load(npsavedir+"/cdscapes/dscape_NGC+WDmed.npy")
        ...
        for k, fname in enumerate(expfile_lst): 
            ...
            for n in np.arange(0,len(upedges),2):       
                ...
                for p, offsets_i in enumerate([polynoms, splines, poly1d_lst, spline1d_lst]):
                    ...
                    O_E_grad, O_E = polfun.detslitdiffnorm([Ecorr,Ocorr], 
                    ...
                    '''
                    # Remove chip edges
                    offsx_i = offsx_i[:, chip_xyranges[0][0]:chip_xyranges[0][1]]
                    offsy_i = offsy_i[:, chip_xyranges[0][0]:chip_xyranges[0][1]]
                    
                    # Determine median whole-pixel offsets for each slit
                    ...
                    O_E = polfun.align_slits([E,O], [[offsfactx, offsfacty]], detoffs=False)
