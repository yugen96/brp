...
def QvsU(fig, ax, QU, PlPhi, PLPHI=np.zeros(4), checks=False, plot_inset=False, inset_ax=None, colour='b', tag=None):
...
WD1615BV_PlPhi, WD1615BV_sigmaPlPhi = [[0., 0.], [0., 0.]], [[0., 0.], [0., 0.]]
...
for i, std_dir in enumerate(std_dirs):
    ...
    Bind = np.array(filt=="b_HIGH" for filt in filter_lst)
    Vind = np.array(filt=="v_HIGH" for filt in filter_lst)
    ...
    for j, tpl_name in enumerate(tpl_dirlst): 
        ...
        apRvsPl(ax0, r_range, pixscale, QUPphi0_jqr[2,j,0], sigmaQUPphi0_jqr[2,j,0], esoP=ESO_PL, esosigmaP=ESO_sigmaPL, colour=plotcolour, tag=tpl_name)
        ...
        for n in range(2): 
            ...
            plt.figure(1)
            plt.errorbar(ret_angles, normflux_k, yerr=normfluxerr_k, marker=mark_lst[n], color=plotcolour, label=lbl)
            # Plot cumulative counts vs aperture radius in O and E for standard star in exposure 1
            plt.figure(2)
            plt.errorbar(r_range, cumc, yerr=sigma_cumc, marker = mark_lst[n], color=plotcolour, linestyle='--')
            plt.errorbar(r_range, cumccorr, yerr=sigma_cumccorr, marker=mark_lst[n], color=plotcolour, label=lbl)
        ...
        if i == 0:
            if j == 0:
                axins = zoomed_inset_axes(ax3, 7, loc=2)
                plotinset = True
        else:
            docheck = False
        PlPhi_lst = [QUPphi0_jq[2,j,0], sigmaQUPphi0_jq[2,j,0], QUPphi0_jq[3,j,0], sigmaQUPphi0_jq[3,j,0]]  
        QU_lst = [QUPphi0_jq[0,j,0], sigmaQUPphi0_jq[0,j,0], QUPphi0_jq[1,j,0], sigmaQUPphi0_jq[1,j,0]]
        QvsU(fig3, ax3, QU=QU_lst, PlPhi=PlPhi_lst, colour=plotcolour, PLPHI=[ESO_PL,ESO_sigmaPL,ESO_PHI,ESO_sigmaPHI], checks=docheck, plot_inset=plotinset, inset_ax = axins, tag=tpl_name)
        axlim = PlPhi_lst[0]*3.
        ...
        plt.errorbar(regsdistOE[0]*pixscale, QUPphi0_jq[2,j,:], yerr = sigmaQUPphi0_jq[2,j,:], fmt='o', color=plotcolour, label = tpl_name)
    ...
    with sns.axes_style("whitegrid",{'axes.grid' : False}):
        ...
        image = ax5.imshow(datacor, cmap='Greys', origin='lower', norm=norm)
        plt.colorbar(image)
        # Plot vectors
        Q = ax5.quiver(vectorX, vectorY, vectorU, vectorV, units='x', pivot='mid', width=10,
                       color = '#666699')
    ...
    plt.figure(4)                    
    plt.title(r"Radial LinPol Profile".format(A = std_dir.split("/")[-2]))
    ...
    plt.title(r"Polarization Map {A}".format(A = std_dir.split("/")[-2]))
    plt.xlabel(r'X [arcsec]', size=24)
    plt.ylabel(r'Y [arcsec]', size=24)
    plt.legend(loc = 'best')
    plt.tight_layout()
