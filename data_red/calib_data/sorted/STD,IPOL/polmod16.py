...
for i, std_dir in enumerate(std_dirs):
    ...
    for n in range(2):
        ...
        # Normalized flux rates and errors
        normflux_jkq, normfluxerr_jkq = compute_norm(OEF_jkq[n], sigmaOEF_jkq[n], ax=1)
        normfluxOE_jkq.append( normflux_jkq ), normfluxerrOE_jkq.append( normfluxerr_jkq )
    ...
    for J, tpl_name in enumerate(tpl_dirlst): 
        ...
        for n in range(2): 
            ...
            # Average over band filters taking 
            ...
            # Second normflux calculation strategy: average over O and E instead of over all templates
            normfluxOE2_jk, normfluxerrOE2_jk = compute_norm(OEF_jkq[0:2,:,:,0], 
                                                             sigmaOEF_jkq[0:2,:,:,0], 
                                                             ax=0)
            
            '''                                                 
            # Average over templates
            normfluxOE2_k = np.average(normfluxOE2_jk[:,filtermask,:], 
                                       weights=1./normfluxerrOE2_jk[:,filtermask,:]**2, 
                                       axis=1)
            normfluxerrOE2_k = np.sqrt(1. / np.sum(1./normfluxerrOE2_jk[:,filtermask,:]**2,
                                                   axis=1))  
            '''
            ...
            if filter_lst[j] == "v_HIGH":
                theta = np.linspace(0,np.pi,100)
                plt.plot(theta*180/np.pi/2., 1. + (-1)**(n+1) * (max(abs(normflux_k)) - 1.)*np.cos(theta*2.), color='0.3', label="")

            plt.errorbar(ret_angles, normfluxOE2_jk[n,j], yerr=normfluxerrOE2_jk[n,j], 
                         color=plotcolour, linestyle="", 
                         marker=mark_lst[n], markersize=10., label=lblfig1_2)
            #TODO #TODO #TODO #TODO #TODO #TODO  
        ...
        plt.errorbar(r_range, cumccorr, yerr=sigma_cumccorr, marker='o', color=plotcolour, label=lbl)  
        #TODO #TODO #TODO       
