...
for i, std_dir in enumerate(std_dirs): 
    ...
    skip_lst = np.zeros(len(std_dirs))
    ...
    for j, tpl_name in enumerate(tpl_dirlst):
        ...
        for k, f in enumerate(expfile_lst):
            ...
            if (f.endswith("fits")) and (len(expfile_lst) == 4):
                ...
                daofind = DAOStarFinder(fwhm=1.0, threshold=500.*std)
                ...
                for q, xc in enumerate(list(set(xcs_lst))):
                    ...
                    F_2dict.update({xc: F_3lst}), sigmaF_2dict.update({xc: sigmaF_3lst})
            print("\t\t\tF_2dict:\t{}".format(len(F_2dict))) 
            # Append second sublist to first sublist
            F_1lst.append(F_2dict), sigmaF_1lst.append(sigmaF_2dict)
    ...
    print("SHAAAAAAAAAAAAPE:\t\t{}".format(F_0lst.shape))
    
    
'''   
    #### Plots ####    
    for j in range(len(tpl_dirlst)):
        least_expno = least_expnos[j]
        xcoords = F_0lst[j-skip_lst[i],least_expno,0].keys()

        for xc in xcoords:
            
            # Compute Stokes variables
            Q_r = 0.5 * F_0lst[j,0,:][xc] - 0.5 * F_0lst[j,2,:]
