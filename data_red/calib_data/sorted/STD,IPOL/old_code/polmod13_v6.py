...
ret_angles = np.arange(0.0, 90.0, 22.5)
...
for i, std_dir in enumerate(std_dirs): 
    ...
    for j, tpl_name in enumerate(tpl_dirlst[skips::]): # TODO imbed in main loop
        ...
        if filter_lst[j] == "b_HIGH":
            plt.figure(1)
            plt.errorbar(phi0_q, Q_q, yerr = sigmaQ_q, fmt='o', color = Bcolor_lst[b], label = tpl_name)
            plt.figure(2)
            plt.errorbar(phi0_q, U_q, yerr = sigmaU_q, fmt='o', color = Bcolor_lst[b], label = tpl_name)
            plt.figure(3)
            plt.errorbar(U_q, Q_q, xerr = sigmaU_q, yerr = sigmaQ_q, fmt='o', color = Bcolor_lst[b], label = tpl_name)
            b += 1
        if filter_lst[j] == "v_HIGH":
            plt.figure(1)
            plt.errorbar(phi0_q, Q_q, yerr = sigmaQ_q, fmt='o', color = Vcolor_lst[b], label = tpl_name)
            plt.figure(2)
            plt.errorbar(phi0_q, U_q, yerr = sigmaU_q, fmt='o', color = Vcolor_lst[b], label = tpl_name)
            plt.figure(3)
    ...
    plt.figure(1)
    savedir = plotdir + '/' + std_dir.split("/")[-2] +'phi0vsQ'
    ...
    plt.figure(2)
    savedir = plotdir + '/' + std_dir.split("/")[-2] +'phi0vsU'
    if not os.path.exists(savedir):
        os.chdir(plotdir)
        os.makedirs(savedir)
        os.chdir(stddatadir)
