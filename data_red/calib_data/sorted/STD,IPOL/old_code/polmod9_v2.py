...
for i, std_dir in enumerate(std_dirs): 
    ...
    F_0lst, sigmaF_0lst = [], []
    # A variable for temporarily storing the number of found stars within a specific exposure and a 1D lst for storing the exposures containing the least amount of stars per template
    starnr = 1e18
    leaststars_expnos = []
    ...
    for j, tpl_name in enumerate(tpl_dirlst):
        ...
        for k, f in enumerate(expfile_lst):
            ...
            if (f.endswith("fits")) and (len(expfile_lst) == 4):
                ...
                # Search for the exposure with the least amount of stars
                if len(xcoords) < starnr:
                    starnr = len(xcoords)
                ...
                for l, R in enumerate(r_range):  
                    ...
                    for q, xc in enumerate(list(set(xcoords))):              
                        ...
                        F_3dict[xc] = F, sigmaF_3dict[xc] = sigmaF
                        print(len(F_3dict))
                    # Append the third sublist to the second sublist
                    F_2lst.append(F_3dict), sigmaF_2lst.append(sigmaF_3dict)     
