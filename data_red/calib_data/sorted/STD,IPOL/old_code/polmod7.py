...
for i in range(len(std_dirs)): 
    ...
    for j in range(len(tpl_dirlst)):
        ...
        r_range = np.arange(0.1, 6.1, 0.1)
        # Lists for storing the different Q and U values of each exposure within each template for each aperture radius 
        F_lst = np.zeros([len(expfile_lst), len(r_range)])
        sigmaF_lst = np.zeros([len(expfile_lst), len(r_range)])
        ...
        for k in range(len(expfile_lst)):
            ...
            if (f.endswith("fits")) and (len(expfile_lst) == 4):
                ...
                for l in range(len(r_range)):      
                    ...
                    F_lst[k,l] = F
                    sigmaF_lst[k,l] = sigmaF
                    
                    
                
        # Compute Stokes variables
        Q_r = 0.5 * F_lst[0,:] - 0.5 * F_lst[2,:]
        U_r = 0.5 * F_lst[1,:] - 0.5 * F_lst[3,:]
        # Compute standard deviations
        sigmaQ_r = 0.5 * np.sqrt(sigmaF_lst[0,:]**2 + sigmaF_lst[2,:]**2)
        sigmaU_r = 0.5 * np.sqrt(sigmaF_lst[1,:]**2 + sigmaF_lst[3,:]**2)
        ...
        #plt.plot(r_range*pixscale, P_r)
        plt.errorbar(r_range*pixscale, P_r, yerr = sigmaP_r, fmt='o', label=filt_name)
