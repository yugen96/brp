...
def find_star(coord, data_array, window_size):
    
    # Read data_array shape and the aproximate pixel's x- and y-coordinates
    aprox_x, aprox_y = coord[1], coord[0]
    ...
    countmax = 0
    for col in np.arange(xmin, xmax, 1):
        for row in np.arange(ymin, ymax, 1):
            
            pix_val = data_array[row,col]
            if pix_val > countmax:
                countmax = pix_val
                center = [row,col]
...
r_range = np.arange(1, 11, 1) #[pixel]
...
compute_anew = True
# Aproximate x-coordinates of selection of stars within CHIP1 of 'Vela1_95coord1' and 'WD1615_'
aprox_xc = [[1034, 1177, 804, 319, 281, 414, 253, 531, 1384, 1583, 1179, 1294, 1501], 
            [1039, 248, 240, 362, 599, 365, 702, 756, 801, 1055, 1186, 1330, ,1132, 1222, 1395, 1413, 1517, 1618, 1649, 1655, 1643, 1632, 1608]] #[pixel]
...
for i, std_dir in enumerate(std_dirs): 
    ...
    filter_lst, STD_indices0, STD_xcoords0 = [], [], []
    for j, tpl_name in enumerate(tpl_dirlst):
        ...
        for k, f in enumerate(expfile_lst):
            ...
            if (f.endswith("fits")) and (len(expfile_lst) == 4):
                ...
                #TODO REPLACE!!!!!!!!!!!!!!!!!!!!!!!!!!!
                #TODO MAKE SELECTION OF STARS YOURSEL! STAR PIXEL COORDINATES DON'T SEEM TO VARY SIGNIFICANTLY OVER DIFFERENT TEMPLATES AND/OR EXPOSURES
                ...
                for q, xc in enumerate(list(set(xcs_lst))):
                    ...
                    if xc in np.arange(aprox_xc[i,0]-7, aprox_xc[i,0]+8, 1):
            ...
            # Append STD_ind to first sublist
            STD_indices1.append(STD_ind), STD_xcoords1.append(STD_xcoord)
        ...
        # Append filter name to filter_lst and STD_indices1 to STD_indices0
        filter_lst.append(filt_name), STD_indices0.append(STD_indices1), STD_xcoords0.append(STD_xcoords)
    ...
    # Save STD_indices, STD_xcoords and filt_lst
    np.save(savedir + "/STD_indices0.npy", STD_indices0)
    np.save(savedir + "/STD_xcoords0.npy", STD_xcoords0)
...
for i, std_dir in enumerate(std_dirs): 
    ...
    # Load STD_ind
    STD_indices0 = np.load("STD_indices0.npy")
    STD_xcoords0 = np.load("STD_xcoords0.npy")
    ...
    for j, tpl_name in enumerate(tpl_dirlst[skip_lst[i]::]):
        STD_ind = STD_indices0[j,:]
        STD_xcoords = STD_xcoords0[j,:]
