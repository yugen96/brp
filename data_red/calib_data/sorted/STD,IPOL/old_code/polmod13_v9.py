...
for i, std_dir in enumerate(std_dirs): 
    ...
    for j, tpl_name in enumerate(tpl_dirlst[skip_lst[i]::]):
        ...
        if filter_lst[j] == "b_HIGH":
            ...
            if i == 0:
                tempPl1, tempPl2 = np.tile(0.0750, len(tempr)), np.tile(0.0760, len(tempr))
            ...
            plt.figure(1)
            plt.errorbar(phi0_jqr[j,0,std_rad-1], Q_jqr[j,0,std_rad-1], yerr = sigmaQ_jqr[j,0,std_rad-1], fmt='o', color = Bcolor_lst[b], label = tpl_name)
            
            # phi0 vs U for standard star in all templates
            plt.figure(2)
            plt.errorbar(phi0_jqr[j,0,std_rad-1], U_jqr[j,0,std_rad-1], yerr = sigmaU_jqr[j,0,std_rad-1], fmt='o', color = Bcolor_lst[b], label = tpl_name)
            
            # U vs Q for all stars and all templates
            plt.figure(3)
            plt.errorbar(U_jqr[j,:,std_rad-1], Q_jqr[j,:,std_rad-1], xerr = sigmaU_jqr[j,:,std_rad-1], yerr = sigmaQ_jqr[j,:,std_rad-1], fmt='o', color = Bcolor_lst[b], label = tpl_name)
            
            # The normalized ordinary and extraordinary flux rates as function of retarder waveplate angle
            plt.figure(4)
            normO = np.average(O_0lst[j,:,0,-9], weights = 1. / sigmaO_0lst[j,:,0,std_rad-1]**2)
            ...
            tempO = sigmaO_0lst[j,:,0,std_rad-1]**2 + (O_0lst[j,:,0,-9]**2 * normO_err**2) / normO**2
            plotO_err = 1/normO * np.sqrt(tempO)
            
            plotE = E_0lst[j,:,0,std_rad-1]/normE
            tempE = sigmaE_0lst[j,:,0,std_rad-1]**2 + (E_0lst[j,:,0,-9]**2 * normE_err**2) / normE**2
            ...
            #TODO CODE THIS!!!
            # Plot linear polarization degree versus distance from FOV center
            plt.figure(5)
            ...
            plt.plot(dists, fit_P, color = Bcolor_lst[b])
        ...
        if filter_lst[j] == "v_HIGH":
            ...
            # phi0 vs Q for standard star in all templates
            plt.figure(1)
            plt.errorbar(phi0_jqr[j,0,std_rad-1], Q_jqr[j,0,std_rad-1], yerr = sigmaQ_jqr[j,0,std_rad-1], fmt='o', color = Vcolor_lst[v], label = tpl_name)

            # phi0 vs U for standard star in all templates            
            plt.figure(2)
            plt.errorbar(phi0_jqr[j,0,std_rad-1], U_jqr[j,0,std_rad-1], yerr = sigmaU_jqr[j,0,std_rad-1], fmt='o', color = Vcolor_lst[v], label = tpl_name)

            # U vs Q for all stars and all templates            
            plt.figure(3)
            plt.errorbar(U_jqr[j,:,std_rad-1], Q_jqr[j,:,std_rad-1], xerr = sigmaU_jqr[j,:,std_rad-1], yerr = sigmaQ_jqr[j,:,std_rad-1], fmt='o', color = Vcolor_lst[v], label = tpl_name)
            ...
            model_P = 0.057 * rs**2
            plt.plot(rs, model_P, color='g', label = tpl_name)
    ...
    plt.xlabel(r'radius [arcsec]', size=24)
    ...
    plt.show()  
    plt.close(0), plt.close(1), plt.close(2), plt.close(3), plt.close(4), plt.close(5), plt.close(6)
