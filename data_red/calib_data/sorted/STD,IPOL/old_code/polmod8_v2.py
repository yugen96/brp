...
for i in range(len(std_dirs)): 
    ...
    for j in range(len(tpl_dirlst)):
        ...
        for k in range(len(expfile_lst)):
            ...
            if (f.endswith("fits")) and (len(expfile_lst) == 4):
                ...
                xcoords, ycoords = [int(xc) for xc in xcoords], [int(yc) for yc in ycoords]
                ...
                for l, R in enumerate(r_range):    
                    ...
                    for q in range(len(xcoords)):
                        ...
                        if xcoords[q] in np.arange(aprox_xc[i]-8, aprox_xc[i]+7, 1):  

                            # Compute cumulative counts within aperture
                            apsum = aper.apersum(data, xcoords[q], ycoords[q], R)
    ...
    #### Plots ####
        ...
        '''
        # Q/I vs U/I
        plt.errorbar(U_r, Q_r, xerr = sigmaU_r, yerr = sigmaQ_r, fmt='o')
        plt.title(r"Q/I vs U/I {A} {B}".format(A = std_dir.split("/")[-2], B = tpl_name))
