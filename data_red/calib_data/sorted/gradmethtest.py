...
def offsetopt_cd(O, E, crange, drange, center, R, anRmin, anRmax, savetofits=False,
    ...
    plt.plot(np.arange(0,len(grady[:,15])), grady[:,15])
    plt.xlabel("Y", fontsize=20), plt.ylabel("gradient", fontsize=20)
    plt.savefig(savedir+"grady.png")
    plt.show()
    plt.close()
    
    savefits(grady, savedir, "grady")
    
    # Diagnostic plot
    f, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True)
    dat1 = ax1.imshow(slitdiff, origin='lower', cmap="afmhot")
    dat2 = ax2.imshow(gradx, origin='lower', cmap="afmhot")
    dat3 = ax3.imshow(grady, origin='lower', cmap="afmhot")
    plt.colorbar(dat1)
    plt.show()
    plt.close()
    ...
    if savetofits:
        ...
        dat1 = ax1.imshow(slitdiff, vmin=-0.3, vmax=0.3, origin='lower', cmap="afmhot")
        dat2 = ax2.imshow(gradopt, vmin=-0.3, vmax=0.3, origin='lower', cmap="afmhot")
        plt.colorbar(dat1)
        plt.show()
        plt.close()
...
# Set test Gaussian
testOgauss = gaussian2d([xgrid,ygrid], 45, 45, 0, 2, 2, 3).reshape(tabularasa.shape)
testEgauss = gaussian2d([xgrid,ygrid], 45, 45.3, 0, 2, 2, 3).reshape(tabularasa.shape)
...
gradopt_old, Qopt, opt_cd = offsetopt_cd(testEgauss, testOgauss, crange, drange,
                            [45,45], 10, 12, 2*12, savetofits=True, 
                            savedir=imdir+"/gradmethtest", gradoptname="gradopttest2")
