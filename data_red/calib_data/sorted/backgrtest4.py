...
def offsetopt_cd(O, E, crange, drange, center, R, anRmin, anRmax, savetofits=False,
...
backgrcents, teststarno = [[56,87], [55,87], [53,57], [55,82], [69,48]], 0
...
for starno, starpar in enumerate(star_lsts[0]):
    ...
    # Save original O-E
    savefits(embedO-embedE, imdir+"/backgrtest_star{}".format(starno+1), "slitdiff_star{}".format(starno+1)) 
    ...
    # Determine the stellar counts in O and E
    ...
    bckgO_reg, bckgO_reg_err = apersum_old(embedO, backgrcent[0], backgrcent[1], 15)
    bckgE_reg, bckgE_reg_err = apersum_old(embedE, backgrcent[0], backgrcent[1], 15)
    bckgO_an, bckgO_an_err = ansum(embedO, embedOcent[0], embedOcent[1], starpar[3], 2*starpar[3])
    bckgE_an, bckgE_an_err = ansum(embedE, embedOcent[0], embedOcent[1], starpar[3], 2*starpar[3])
    ...
    savefits(Ocorr_reg, imdir+"/backgrtest_star{}".format(starno+1), "Ocorr_reg_star{}".format(starno+1))
    savefits(Ecorr_reg, imdir+"/backgrtest_star{}".format(starno+1), "Ecorr_reg_star{}".format(starno+1))
    savefits(Ocorr_an, imdir+"/backgrtest_star{}".format(starno+1), "Ocorr_an_star{}".format(starno+1))
    savefits(Ecorr_an, imdir+"/backgrtest_star{}".format(starno+1), "Ecorr_an_star{}".format(starno+1))
    # Determine the stellar counts in O and E for annulus background corrected image
    Ostar_corran, Ostar_corran_err = apersum_old(Ocorr_an, embedOcent[0], embedOcent[1], starpar[3])
    Estar_corran, Estar_corran_err = apersum_old(Ecorr_an, embedOcent[0]+offsets[starno][0], 
                                           embedOcent[1]+offsets[starno][1], starpar[3])
    
    '''                               
    plt.figure()
    plt.imshow(Ocorr_an, origin='lower', cmap='rainbow')
    plt.scatter(embedOcent[0], embedOcent[1], s=30, c='k')
    plt.scatter(backgrcent[0], backgrcent[1], s=30, c='k')
    plt.show()
    plt.close()   
    ...
    bckgO_corran_reg, bckgO_corran_reg_err = apersum_old(Ocorr_an, backgrcent[0], backgrcent[1], 15)
    bckgE_corran_reg, bckgE_corran_reg_err = apersum_old(Ecorr_an, backgrcent[0], backgrcent[1], 15)
    bckgO_corran_an, bckgO_corran_an_err = ansum(Ocorr_an, embedOcent[0], embedOcent[1], starpar[3], 2*starpar[3])
    bckgE_corran_an, bckgE_corran_an_err = ansum(Ecorr_an, embedOcent[0], embedOcent[1], starpar[3], 2*starpar[3])
    ...
    savefits(slitdiff_regcorr, imdir+"/backgrtest_star{}".format(starno+1), "slitdiff_regcorr_star{}".format(starno+1))
    savefits(slitdiff_ancorr, imdir+"/backgrtest_star{}".format(starno+1), "slitdiff_anncorr_star{}".format(starno+1))    
    # Compute left-over background signals
    bckgdiff_regcorr, bckgdiff_regcorr_err = apersum_old(slitdiff_regcorr, backgrcent[0], backgrcent[1], 15)
    bckgdiff_ancorr, bckgdiff_ancorr_err = apersum_old(slitdiff_ancorr, backgrcent[0], backgrcent[1], 15)
    bckgdiff_regcorrABS, bckgdiff_regcorr_errABS = apersum_old(np.abs(slitdiff_regcorr), backgrcent[0], backgrcent[1], 15)
    bckgdiff_ancorrABS, bckgdiff_ancorr_errABS = apersum_old(np.abs(slitdiff_ancorr), backgrcent[0], backgrcent[1], 15)
    ...
    # NORMAL SCALE
    oldres, oldres_err = apersum_old(gradopt_old, embedOcent[0], embedOcent[1], 5)
    newres_reg, newres_reg_err = apersum_old(gradopt_reg, embedOcent[0], embedOcent[1], 5) 
    newres_an, newres_an_err = apersum_old(gradopt_an, embedOcent[0], embedOcent[1], 5) 
    # ABSOLUTE SCALE
    oldresABS, oldresABS_err = apersum_old(np.abs(gradopt_old), 
                                     embedOcent[0], embedOcent[1], 5)
    newres_regABS, newres_regABS_err = apersum_old(np.abs(gradopt_reg), 
                                             embedOcent[0], embedOcent[1], 5) 
    newres_anABS, newres_anABS_err = apersum_old(np.abs(gradopt_an), 
                                           embedOcent[0], embedOcent[1], 5) 

    # Determine the background flux in derived gradient method optima images
    # NORMAL SCALE
    bckgOPT_old, bckgOPT_old_err = apersum_old(gradopt_old, backgrcent[0], backgrcent[1], 15) 
    bckgOPT_reg, bckgOPT_reg_err = apersum_old(gradopt_reg, backgrcent[0], backgrcent[1], 15) 
    bckgOPT_an, bckgOPT_an_err = apersum_old(gradopt_an, backgrcent[0], backgrcent[1], 15)    
    # ABSOLUTE SCALE 
    bckgOPT_oldABS, bckgOPT_old_errABS = apersum_old(np.abs(gradopt_old), backgrcent[0], backgrcent[1], 15) 
    bckgOPT_regABS, bckgOPT_reg_errABS = apersum_old(np.abs(gradopt_reg), backgrcent[0], backgrcent[1], 15) 
    bckgOPT_anABS, bckgOPT_an_errABS = apersum_old(np.abs(gradopt_an), backgrcent[0], backgrcent[1], 15) 
    ...
    aperdata = np.round([Ostar, bckgO_reg, bckgO_an, Estar, bckgE_reg, bckgE_an])
    ...
    # Write annulus background corrected statistics
    savefile1.write("\n\n")
    savefile1.write(" \t&\t \\textbf{type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \hline \n")
    types = ["star aper", "sky aper", "sky annul", "star aper", "sky aper", "sky annul"]
    R = [str(starpar[3]), str(2*starpar[3]), 15, str(starpar[3]), str(2*starpar[3]), 15]
    aperdata = np.round([Ostar_corran, bckgO_corran_reg, bckgO_corran_an, 
                         Estar_corran, bckgE_corran_reg, bckgE_corran_an])
    aperdata_err = np.round([Ostar_corran_err, bckgO_corran_reg_err, bckgO_corran_an_err, 
                             Estar_corran_err, bckgE_corran_reg_err, bckgE_corran_an_err], 2)
    for i in range(6):
        print(types[i])
        #savefile1.write("   \t&\t {A1} \t&\t {A2} \t&\t {B} \t&\t {C} \t&\t {D}\\\\ \n".format(A1=types[i], A2=R[i], B=aperdata[i][0], C=aperdata[i][1],D=aperdata[i][2]))
        savefile1.write("   \t&\t {A1} \t&\t {A2} \t&\t ${B}\\pm{Berr}$ \t&\t ${C}\\pm{Cerr}$ \t&\t ${D}$\\\\ \n".format(A1=types[i], A2=R[i], B=aperdata[i][0], Berr=aperdata_err[i][0], 
    ...
    savefile1.write("\n\n\n\n\n")
    savefile1.write("Left-over backgrounds in the background corrected slit difference images\n")
    ...
    savefile1.write("\n\n\n\n\n")
    #savefile1.write(" \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\hat{se}_{\\Sigma}$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{$\\hat{se}_{\\mu}$ [ADU]} \t&\t \\textbf{MD [ADU]} \t&\t \\textbf{$\\hat{se}_{MD}$ [ADU]} \\\\ \n")
    savefile1.write("Height of the residual in aperture of 5 pixels for uncorrected, reg corrected and an corrected images, both normally scaled and absolutely scaled.\n")
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \n")   
    types = ["uncorrected", "regional sky corr.", 
             "annul. sky corr.", "$|\\text{uncorrected}|$", 
             "$|\\text{regional sky corr.}|$", "$|\\text{annul. sky corr.}|$"]
    aperdata = np.round([oldres, newres_reg, newres_an, oldresABS, newres_regABS, newres_anABS], 2)
    aperdata_err = np.round([oldres_err, newres_reg_err, newres_an_err, 
                             oldresABS_err, newres_regABS_err, newres_anABS_err], 2)
    for i in range(6):
