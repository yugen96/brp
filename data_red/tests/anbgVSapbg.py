...
for starno, starpar in enumerate(star_lsts[0]):
    ...
    O_E_BGANNstarLST, O_E_BGANNstarerrLST = [], []
    O_E_BGANNregLST, O_E_BGANNregerrLST = [], []
    O_E_BGANNannLST, O_E_BGANNannerrLST = [], []
    anR_MAXrange = np.arange(starpar[3], 3*starpar[3], int(0.2*starpar[3]))
    for anR_MAX in anR_MAXrange:
        print(anR_MAX)
        ...
        bckgO_an, bckgO_an_err, N_bckganpix = ansum(embedO, embedOcent[0], embedOcent[1], starpar[3], anR_MAX)
        bckgE_an, bckgE_an_err, _ = ansum(embedE, embedOcent[0], embedOcent[1], starpar[3], anR_MAX)
        ...
        bckgO_corran_an, bckgO_corran_an_err, _ = ansum(Ocorr_an, embedOcent[0], embedOcent[1], starpar[3], anR_MAX)
        bckgE_corran_an, bckgE_corran_an_err, _ = ansum(Ecorr_an, embedOcent[0], embedOcent[1], starpar[3], anR_MAX)
        ...
        O_E_BGANNstarLST.append(O_E_BGANNstar), O_E_BGANNstarerrLST.append(O_E_BGANNstarerr)   
        O_E_BGANNreg, O_E_BGANNregerr, _ = apersum_old(slitdiff_ancorr, backgrcent[0], backgrcent[1], 15)  
        O_E_BGANNregLST.append(O_E_BGANNreg), O_E_BGANNregerrLST.append(O_E_BGANNregerr)      
        O_E_BGANNann, O_E_BGANNannerr, _ = ansum(slitdiff_ancorr, embedOcent[0], embedOcent[1], starpar[3], anR_MAX)
        O_E_BGANNannLST.append(O_E_BGANNann), O_E_BGANNannerrLST.append(O_E_BGANNannerr)
        
        
        
        # Compute expected residual sum
        ...
        gradopt_old, Qopt, opt_cd = offsetopt_cd(embedO, embedE, crange, drange,
                                    ...
                                    savetofits=True, savedir=imdir+"/backgrtest_star{}".format(starno+1), 
        ...
        os.chdir(savedir)                
        savefile1 = open("tables_anbgVSapbg_star{}anR_MAX{}".format(starno+1, anR_MAX),'w')                                    
        ...
        R = np.tile([starpar[3], 15, str(starpar[3])+"-"+str(anR_MAX)], 9)
        aperdata = np.round([O_E_BGANNstar, O_E_BGANNreg, O_E_BGANNann], 2)
        aperdata_err = np.round([O_E_BGANNstarerr, O_E_BGANNregerr, O_E_BGANNannerr], 2)
        for i in range(3):
            print(types[i])
            if (i+1)%3 == 1:
        ...
        savefile1.write("\\hline \hline \n")
        rowcatgs = ["O - E - $\\Delta \\text{bg}_{\\text{annul}}$ - grad.", ""]
        types = np.tile(["star aper", "sky aper"], 2)
        R = np.tile([starpar[3], 15], 1)
        aperdata = np.round([newres_anEXPECT, bckgregOPT_anEXPECT], 2)
        ...
        aperdata = np.round([newres_an, bckgregOPT_an], 2)
        aperdata_err = np.round([newres_an_err, bckgregOPT_an_err], 2)
        ...
        types = np.tile(["star aper", "sky aper"], 3)
        R = np.tile([starpar[3], 15], 3)
        aperdata = np.round([newres_anABS, bckgregOPT_anABS], 2)
        aperdata_err = np.round([newres_anABS_err, bckgregOPT_anABS_err], 2)
    ...
    O_E_BGANNstarARR, O_E_BGANNregARR, O_E_BGANNannARR = np.asarray([O_E_BGANNstarLST, O_E_BGANNregLST, O_E_BGANNannLST])
    
    plt.figure()
    plt.plot(anR_MAXrange[3::], O_E_BGANNstarARR[3::,0], color='b', marker="o", label=r"star aper")
    plt.plot(anR_MAXrange[3::], O_E_BGANNregARR[3::,0], color='g', marker="s", label=r"sky aper")
    plt.plot(anR_MAXrange[3::], O_E_BGANNannARR[3::,0], color='r', marker="v", label=r"sky annul")
    plt.xlabel(r"$R_{max}$ [pix]", fontsize=20), plt.ylabel(r"Counts [ADU]", fontsize=20)
    plt.title(r"$\Sigma \text{ for }O-E-\Delta bg_{ann}$")
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(plotdir+"/annsizetest_sigma")
    plt.show()
    plt.close()
    
    plt.figure()
    plt.plot(anR_MAXrange[3::], O_E_BGANNstarARR[3::,2], color='b', marker="o", label=r"star aper")
    plt.plot(anR_MAXrange[3::], O_E_BGANNregARR[3::,2], color='g', marker="s", label=r"sky aper")
    plt.plot(anR_MAXrange[3::], O_E_BGANNannARR[3::,2], color='r', marker="v", label=r"sky annul")
    plt.xlabel(r"$R_{max}$ [pix]", fontsize=20), plt.ylabel(r"Counts [ADU]", fontsize=20)
    plt.title(r"MD for $O-E-\Delta bg_{ann}$")
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(plotdir+"/annsizetest_MD")
    plt.show()
    plt.close()
