...
def offsetopt_cd(O, E, crange, drange, center, R, anRmin, anRmax, 
    ...
    plt.figure(8)
    plt.imshow(grady, origin='lower', interpolation='none')
    plt.colorbar()
    plt.show()
    plt.close()    
    ...
    for c in crange:
        for d in drange:
            ...
            '''
            plt.imshow(temp, origin='lower', interpolation='none')
            plt.colorbar()
            plt.show()
            plt.close()  
            '''
    ...
    if savetofits:
        ...
        plt.figure(10)
        plt.imshow(aligned, origin='lower', interpolation='none')
        plt.colorbar()
        plt.show()
        plt.close()
        
        saveim_png(aligned, imdir+"/offsetopt/tests", "cdoffset")
        '''
...
def main():
    ...
    z = 20*(x-.4)**2 + 10*(y-.4)**2 + 3*(x-.4)**3 + 5*(y-.4) + np.random.random(numdata)
    print(x)
