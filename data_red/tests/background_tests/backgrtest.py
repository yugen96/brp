...
def apersum_old(image, px, py, r):
    ...
    return [apsum, (apsum/apcount), np.median(pixvals)]
...
def offsetopt_cd(O, E, crange, drange, center, R, anRmin, anRmax, savetofits=False,
    ...
    for c in crange:
        for d in drange:
            ...
            Qlst = apersum_old(temp, center[0], center[1], R)
            Q = Qlst[0]
            
            savefits(np.sqrt(temp), savedir+"/ABS", 
                     "ABStestc{}d{}".format(np.round([c,d],3)[0], np.round([c,d],3)[1]))
...
for starno, starpar in enumerate(star_lsts[0]):
    ...
    Oold = apersum_old(embedO, embedOcent[0], embedOcent[1], starpar[3])
    Eold = apersum_old(embedE, embedOcent[0], embedOcent[1], starpar[3])
    backgrOold = apersum_old(embedO, 60, 45, starpar[3])
    backgrEold = apersum_old(embedE, 60, 45, starpar[3])
    # Subtract median background from the O and E frames
    embedOcorr = embedO - backgrOold[2]
    embedEcorr = embedE - backgrEold[2]
    savefits(embedOcorr, imdir+"/backgrtest", "Ocorr_star1")
    savefits(embedEcorr, imdir+"/backgrtest", "Ecorr_star1")
    ...
    Qopt, opt_cd = offsetopt_cd(embedOcorr, embedEcorr, crange, drange,
                                embedOcent, starpar[3], starpar[3], 2*starpar[3],
                                savetofits=True, savedir=imdir+"/backgrtest", 
    ...
    # Determine the stellar counts in O and E using a sky anulus subtraction
    Onew, Onew_an = apersum(embedO, embedOcent[0], embedOcent[1], 
                            starpar[3], starpar[3], 2*starpar[3])
    Enew, Enew_an = apersum(embedE, embedOcent[0], embedOcent[1], 
                            starpar[3], starpar[3], 2*starpar[3])
    backgrOnew, backgrOnew_an = apersum(embedO, 60, 45, starpar[3], starpar[3], 2*starpar[3])
    backgrEnew, backgrEnew_an = apersum(embedE, 60, 45, starpar[3], starpar[3], 2*starpar[3])
    ...
    savefile1.write(" \t&\t sum \t&\t mean \t&\t median \\\\ \n")
    savefile1.write("\\hline \n")
    # Without sky anulus
    names = ["Oold", "backgrOold", "Eold", "backgrEold"]
    aperdata = [Oold, backgrOold, Eold, backgrEold]
    for i in range(4):
        savefile1.write("{A} \t&\t {B} \t&\t {C} \t&\t {D} \\\\ \n".format(A=names[i], B=aperdata[i][0], C=aperdata[i][1], D=aperdata[i][2]))
        
    savefile1.write("\n\n\n\n\n")
    
    

    savefile1.write(" \t&\t skycorr\_sum \t&\t sum \t&\t mean \t&\t median \\\\ \n")
    savefile1.write("\\hline \n")   
    # with sky anulus 
    names = ["Onew", "backgrOnew", "Enew", "backgrEnew"]
    aperdata = [Onew, backgrOnew, Enew, backgrEnew]
    andata = [Onew_an, Enew_an, backgrOnew_an, backgrEnew_an]
    for i in range(4):
        savefile1.write("{A} \t&\t {B} \t&\t {C} \t&\t {D} t&\t {E}\\\\ \n".format(A=names[i], B=aperdata[i][0], C=aperdata[i][1], D=aperdata[i][2], E=aperdata[i][3]))
