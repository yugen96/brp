...
for starno, starpar in enumerate(star_lsts[0]):
    ...
    # Save original O-E
    savefits(embedO-embedE, imdir+"/offsetopt/cdscapes12", "slitdiff_star{}".format(starno+1)) 
    savefits(cutoutO, imdir+"/offsetopt/cdscapes12", "O_star{}".format(starno+1))       
    savefits(cutoutE, imdir+"/offsetopt/cdscapes12", "E_star{}".format(starno+1)) 
    ...
    plt.figure()
    plt.imshow(embedO, origin='lower', cmap='rainbow')
    plt.scatter(53, 72, s=30, c='k')
    plt.show()
    plt.close()    
                                           
    bckgO_reg, bckgO_reg_err = apersum_old(embedO, 53, 72, 15)
    bckgE_reg, bckgE_reg_err = apersum_old(embedE, 53, 72, 15)
    ...
    Ostar_corran, Ostar_corran_err = apersum_old(embedO, embedOcent[0], embedOcent[1], starpar[3])
    Estar_corran, Estar_corran_err = apersum_old(embedE, embedOcent[0]+offsets[starno][0], 
                                           embedOcent[1]+offsets[starno][1], starpar[3])
    ...
    bckgO_corran_reg, bckgO_corran_reg_err = apersum_old(Ocorr_an, 53, 72, 15)
    bckgE_corran_reg, bckgE_corran_reg_err = apersum_old(Ecorr_an, 53, 72, 15)
    ...
    bckgdiff_regcorr, bckgdiff_regcorr_err = apersum_old(slitdiff_regcorr, 53, 72, 15)
    bckgdiff_ancorr, bckgdiff_ancorr_err = apersum_old(slitdiff_ancorr, 53, 72, 15)
    bckgdiff_regcorrABS, bckgdiff_regcorr_errABS = apersum_old(np.abs(slitdiff_regcorr), 53, 72, 15)
    bckgdiff_ancorrABS, bckgdiff_ancorr_errABS = apersum_old(np.abs(slitdiff_ancorr), 53, 72, 15)
    ...
    # NORMAL SCALE
    bckgOPT_old, bckgOPT_old_err = apersum_old(gradopt_old, 53, 72, 15) 
    bckgOPT_reg, bckgOPT_reg_err = apersum_old(gradopt_reg, 53, 72, 15) 
    bckgOPT_an, bckgOPT_an_err = apersum_old(gradopt_an, 53, 72, 15)    
    # ABSOLUTE SCALE 
    bckgOPT_oldABS, bckgOPT_old_errABS = apersum_old(np.abs(gradopt_old), 53, 72, 15) 
    bckgOPT_regABS, bckgOPT_reg_errABS = apersum_old(np.abs(gradopt_reg), 53, 72, 15) 
    bckgOPT_anABS, bckgOPT_an_errABS = apersum_old(np.abs(gradopt_an), 53, 72, 15) 
