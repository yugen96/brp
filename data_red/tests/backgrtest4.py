...
def apersum_old(image, px, py, r):
    ...
    rmse = np.sum( np.sqrt((np.array(pixvals) - med)**2) ) / (apcount - 1)
...
def ansum(image, px, py, minAn_r, maxAn_r, itnos=21):
    ...
    return [ansum, av_an, np.median(pixvals_an), sigma_an], [np.sqrt(abs(ansum)), mean_err, med_err], ancount
...
def offsetopt_cd(O, E, crange, drange, center, R, savetofits=False,
    ...
    savefits(gradx, imsavedir, "gradxSTAR"+gradoptname.split("STAR")[1])
    savefits(grady, imsavedir, "gradySTAR"+gradoptname.split("STAR")[1])
    ...
    E_O, Ogrady = (E-O)[:,center[0]], grady[:,center[0]]
    normval1, normval2 = np.max(E_O[~np.isnan(E_O)]), np.max(Ogrady[~np.isnan(Ogrady)])
    print(E_O[~np.isnan(E_O)])
    plt.plot(np.arange(0,O[~np.isnan(E_O)].shape[0]), E_O[~np.isnan(E_O)]/normval1, color="r", marker='x', linestyle='--', label=r"$E-O$")
...
backgrcents, teststarno = [[60,41], [56,87], [55,87], [53,57], [55,82], [69,48], [53,72]], 0
...
for starno, starpar in enumerate(star_lsts[0]):
    ...
    if starno+1 in [1, 4, 8, 14, 16, 20, 28]:
    ...
    plt.scatter(backgrcent[0], backgrcent[1], s=30, c='k')
    plt.axis(xmin=0, xmax=embedO.shape[1], ymin=0, ymax=embedO.shape[0])
    plt.show()
    plt.close()
    ...
    # Save original O-E
    savefits(embedO-embedE, imdir+"/backgrtest_star{}".format(starno+1), "O-E_STAR{}".format(starno+1)) 
    savefits(cutoutO, imdir+"/backgrtest_star{}".format(starno+1), "O_STAR{}".format(starno+1))       
    savefits(cutoutE, imdir+"/backgrtest_star{}".format(starno+1), "E_STAR{}".format(starno+1)) 
    # Save embeded O and E
    savefits(embedO, imdir+"/backgrtest_star{}".format(starno+1), "embedO_STAR{}".format(starno+1)) 
    savefits(embedE, imdir+"/backgrtest_star{}".format(starno+1), "embedE_STAR{}".format(starno+1)) 
    # Save extra offsets to be sure this is the best pixel accurate alignment
    for extradx, extrady in carthprod(range(-1,2),range(-1,2)):
        testEembed = embed(cutoutE, framesize, 
                           offset=np.array(offsets[starno])+np.array([extradx,extrady]),
                           cornerpix=lowlcorn)
        savefits(embedO-testEembed, imdir+"/backgrtest_star{}/pixofftests".format(starno+1), 
                 "extradx{}extrady{}".format(extradx,extrady))
    
    
    
    # Determine the stellar and background statistics in O and E
    Ostar, Ostar_err, N_starpix = apersum_old(embedO, embedOcent[0], embedOcent[1], starpar[3])
    Estar, Estar_err, _ = apersum_old(embedE, embedOcent[0], embedOcent[1], starpar[3])
    bckgO_reg, bckgO_reg_err, N_bckgappix = apersum_old(embedO, backgrcent[0], backgrcent[1], 15)
    bckgE_reg, bckgE_reg_err, _ = apersum_old(embedE, backgrcent[0], backgrcent[1], 15)
    bckgO_an, bckgO_an_err, N_bckganpix = ansum(embedO, embedOcent[0], embedOcent[1], starpar[3], 23)
    bckgE_an, bckgE_an_err, _ = ansum(embedE, embedOcent[0], embedOcent[1], starpar[3], 23)
    ...
    savefits(Ocorr_reg, imdir+"/backgrtest_star{}".format(starno+1), "O-bckg_OaperSTAR{}".format(starno+1))
    savefits(Ecorr_reg, imdir+"/backgrtest_star{}".format(starno+1), "E-bckg_EaperSTAR{}".format(starno+1))
    savefits(Ocorr_an, imdir+"/backgrtest_star{}".format(starno+1), "O-bckg_OannulSTAR{}".format(starno+1))
    savefits(Ecorr_an, imdir+"/backgrtest_star{}".format(starno+1), "E-bckg_EannulSTAR{}".format(starno+1))
    
    # Determine stellar and background statistics in O and E - BG_reg
    Ostar_correg, Ostar_correg_err, _ = apersum_old(Ocorr_reg, embedOcent[0], 
                                                    embedOcent[1], starpar[3])
    Estar_correg, Estar_correg_err, _ = apersum_old(Ecorr_reg, embedOcent[0], 
                                                    embedOcent[1], starpar[3])         
    bckgO_correg_reg, bckgO_correg_reg_err, _ = apersum_old(Ocorr_reg, backgrcent[0], 
                                                            backgrcent[1], 15)
    bckgE_correg_reg, bckgE_correg_reg_err, _ = apersum_old(Ecorr_reg, backgrcent[0], 
                                                            backgrcent[1], 15)
    bckgO_correg_an, bckgO_correg_an_err, _ = ansum(Ocorr_reg, embedOcent[0], 
                                                    embedOcent[1], starpar[3], 23)
    bckgE_correg_an, bckgE_correg_an_err, _ = ansum(Ecorr_reg, embedOcent[0], 
                                                    embedOcent[1], starpar[3], 23)
                                               
    # Determine stellar and background statistics in O and E - BG_ann
    Ostar_corran, Ostar_corran_err, _ = apersum_old(Ocorr_an, embedOcent[0], 
                                                    embedOcent[1], starpar[3])
    Estar_corran, Estar_corran_err, _ = apersum_old(Ecorr_an, embedOcent[0], 
                                                    embedOcent[1], starpar[3])         
    bckgO_corran_reg, bckgO_corran_reg_err, _ = apersum_old(Ocorr_an, backgrcent[0], backgrcent[1], 15)
    bckgE_corran_reg, bckgE_corran_reg_err, _ = apersum_old(Ecorr_an, backgrcent[0], backgrcent[1], 15)
    bckgO_corran_an, bckgO_corran_an_err, _ = ansum(Ocorr_an, embedOcent[0], embedOcent[1], starpar[3], 23)
    bckgE_corran_an, bckgE_corran_an_err, _ = ansum(Ecorr_an, embedOcent[0], embedOcent[1], starpar[3], 23)
    
    
    
    
    # Determine slitdifferences for background corrected images
    slitdiff_old = embedO - embedE
    slitdiff_regcorr = Ocorr_reg - Ecorr_reg
    slitdiff_ancorr = Ocorr_an - Ecorr_an
    savefits(slitdiff_old, imdir+"/backgrtest_star{}".format(starno+1), "O-ESTAR{}".format(starno+1))    
    savefits(slitdiff_regcorr, imdir+"/backgrtest_star{}".format(starno+1), "O-E-bckg_aperSTAR{}".format(starno+1))
    savefits(slitdiff_ancorr, imdir+"/backgrtest_star{}".format(starno+1), "O-E-bckg_annulSTAR{}".format(starno+1))    
    # Compute left-over background signals
    O_Estar, O_Estarerr, _ = apersum_old(slitdiff_old, embedOcent[0], embedOcent[1], starpar[3])
    O_Ereg, O_Eregerr, _ = apersum_old(slitdiff_old, backgrcent[0], backgrcent[1], 15)
    O_Eann, O_Eannerr, _ = ansum(slitdiff_old, embedOcent[0], embedOcent[1], starpar[3], 23)
    O_E_BGREGstar, O_E_BGREGstarerr, _ = apersum_old(slitdiff_regcorr, embedOcent[0], embedOcent[1], starpar[3])
    O_E_BGREGreg, O_E_BGREGregerr, _ = apersum_old(slitdiff_regcorr, backgrcent[0], backgrcent[1], 15)
    O_E_BGREGann, O_E_BGREGannerr, _ = ansum(slitdiff_regcorr, embedOcent[0], embedOcent[1], starpar[3], 23)
    O_E_BGANNstar, O_E_BGANNstarerr, _ = apersum_old(slitdiff_ancorr, embedOcent[0], embedOcent[1], starpar[3])
    O_E_BGANNreg, O_E_BGANNregerr, _ = apersum_old(slitdiff_ancorr, backgrcent[0], backgrcent[1], 15)
    O_E_BGANNann, O_E_BGANNannerr, _ = ansum(slitdiff_ancorr, embedOcent[0], embedOcent[1], starpar[3], 23)
    
    
    
    
    
    # Compute the EXPECTED sum over the old and new residuals
    newres_regEXPECT = np.sqrt(Ostar_err[0]**2 + Estar_err[0]**2 + N_starpix*(bckgO_reg[2] + bckgE_reg[2]))
    bckgregOPT_regEXPECT = np.sqrt(N_bckgappix*(bckgO_reg[2] + bckgE_reg[2]))
    newres_anEXPECT = np.sqrt(Ostar_err[0]**2 + Estar_err[0]**2 + N_starpix*(bckgO_an[2] + bckgE_an[2]))
    bckgregOPT_anEXPECT = np.sqrt(N_bckgappix*(bckgO_an[2] + bckgE_an[2]))  
    ...
    gradopt_old, Qopt, opt_cd = offsetopt_cd(embedO, embedE, crange, drange,
                                ...
                                savetofits=True, imsavedir=imdir+"/backgrtest_star{}".format(starno+1), 
                                pltsavedir=plotdir+"/backgrtest_star{}".format(starno+1),
                                gradoptname="O-E-gradSTAR{}".format(starno+1))
    ...
    gradopt_old, Qopt, opt_cd = offsetopt_cd(embedO, embedE, newcrange, newdrange,
                                ...
                                savetofits=True, imsavedir=imdir+"/backgrtest_star{}".format(starno+1), 
                                pltsavedir=plotdir+"/backgrtest_star{}".format(starno+1),
                                gradoptname="O-E-grad2STAR{}".format(starno+1))
    Qopts.append(Qopt), opts_cd.append(opt_cd)
    print("Qopt, opt_cd:\t\t", Qopt, opt_cd)
    ...
    gradopt_reg, Qopt, opt_cd = offsetopt_cd(Ocorr_reg, Ecorr_reg, crange, drange,
                                ...
                                savetofits=True, imsavedir=imdir+"/backgrtest_star{}".format(starno+1), 
                                pltsavedir=plotdir+"/backgrtest_star{}".format(starno+1),
                                gradoptname="O-E-bckg_aper-gradSTAR{}".format(starno+1))
    Qopts.append(Qopt), opts_cd.append(opt_cd)
    print("Qopt, opt_cd:\t\t", Qopt, opt_cd)
    ...
    gradopt_an, Qopt, opt_cd = offsetopt_cd(Ocorr_an, Ecorr_an, crange, drange,
                                ...
                                savetofits=True, imsavedir=imdir+"/backgrtest_star{}".format(starno+1), 
                                pltsavedir=plotdir+"/backgrtest_star{}".format(starno+1),
                                gradoptname="O-E-bckg_annul-gradSTAR{}".format(starno+1))
    Qopts.append(Qopt), opts_cd.append(opt_cd)
    print("Qopt, opt_cd:\t\t", Qopt, opt_cd)
    ...
    # NORMAL SCALE
    oldres, oldres_err, _ = apersum_old(gradopt_old, embedOcent[0], embedOcent[1], starpar[3])
    newres_reg, newres_reg_err, _ = apersum_old(gradopt_reg, embedOcent[0], embedOcent[1], starpar[3]) 
    newres_an, newres_an_err, _ = apersum_old(gradopt_an, embedOcent[0], embedOcent[1], starpar[3]) 
    # ABSOLUTE SCALE
    oldresABS, oldresABS_err, _ = apersum_old(np.abs(gradopt_old), 
                                           embedOcent[0], embedOcent[1], starpar[3])
    newres_regABS, newres_regABS_err, _ = apersum_old(np.abs(gradopt_reg), 
                                                   embedOcent[0], embedOcent[1], starpar[3]) 
    newres_anABS, newres_anABS_err, _ = apersum_old(np.abs(gradopt_an), 
                                                 embedOcent[0], embedOcent[1], starpar[3]) 
    
    
    # Determine the background flux in derived gradient method optima images
    # NORMAL SCALE
    bckgregOPT_old, bckgregOPT_old_err, _ = apersum_old(gradopt_old, backgrcent[0], backgrcent[1], 15) 
    bckgregOPT_reg, bckgregOPT_reg_err, _ = apersum_old(gradopt_reg, backgrcent[0], backgrcent[1], 15) 
    bckgregOPT_an, bckgregOPT_an_err, _ = apersum_old(gradopt_an, backgrcent[0], backgrcent[1], 15)    
    # ABSOLUTE SCALE 
    bckgregOPT_oldABS, bckgregOPT_oldABS_err, _ = apersum_old(np.abs(gradopt_old), backgrcent[0], backgrcent[1], 15) 
    bckgregOPT_regABS, bckgregOPT_regABS_err, _ = apersum_old(np.abs(gradopt_reg), backgrcent[0], backgrcent[1], 15) 
    bckgregOPT_anABS, bckgregOPT_anABS_err, _ = apersum_old(np.abs(gradopt_an), backgrcent[0], backgrcent[1], 15) 
    ...
    os.chdir(savedir)                
    savefile1 = open("tables_star{}".format(starno+1),'w')                                    
    # O, E, O-bg_[O,aper], E-bg_[E,aper], O-bg_[O,annul], E-bg_[E,annul] --> star aper, sky aper, sky annul\n
    savefile1.write("\n\n\n\n\n")   
    savefile1.write("O, E, O-bg_[O,aper], E-bg_[E,aper], O-bg_[O,annul], E-bg_[E,annul] --> star aper, sky aper, sky annul\n")  
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{Aper Type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \hline \n")
    rowcatgs = ["O", "", "",
                "E", "", "",
                "O-$\\text{bg}_{\\text{O,aper}}$", "", "",
                "E-$\\text{bg}_{\\text{E,aper}}$", "", "", 
                "O-$\\text{bg}_{\\text{O,annul}}$", "", "",
                "E-$\\text{bg}_{\\text{E,annul}}$", "", "",
                "O-E", "", "",
                "O-E-$\\Delta \\text{bg}_{\\text{aper}}$", "", "",
                "O-E-$\\Delta \\text{bg}_{\\text{annul}}$", "", ""]
    types = np.tile(["star aper", "sky aper", "sky annul"], 9)
    R = np.tile([starpar[3], 15, str(starpar[3])+"-"+str(23)], 9)
    aperdata = np.round([Ostar, bckgO_reg, bckgO_an, 
                         Estar, bckgE_reg, bckgE_an, 
                         Ostar_correg, bckgO_correg_reg, bckgO_correg_an, 
                         Estar_correg, bckgE_correg_reg, bckgE_correg_an, 
                         Ostar_corran, bckgO_corran_reg, bckgO_corran_an, 
                         Estar_corran, bckgE_corran_reg, bckgE_corran_an,
                         O_Estar, O_Ereg, O_Eann,
                         O_E_BGREGstar, O_E_BGREGreg, O_E_BGREGann,
                         O_E_BGANNstar, O_E_BGANNreg, O_E_BGANNann], 2)
    aperdata_err = np.round([Ostar_err, bckgO_reg_err, bckgO_an_err, 
                             Estar_err, bckgE_reg_err, bckgE_an_err, 
                             Ostar_correg_err, bckgO_correg_reg_err, bckgO_correg_an_err, 
                             Estar_correg_err, bckgE_correg_reg_err, bckgE_correg_an_err, 
                             Ostar_corran_err, bckgO_corran_reg_err, bckgO_corran_an_err, 
                             Estar_corran_err, bckgE_corran_reg_err, bckgE_corran_an_err,
                             O_Estarerr, O_Eregerr, O_Eannerr,
                             O_E_BGREGstarerr, O_E_BGREGregerr, O_E_BGREGannerr,
                             O_E_BGANNstarerr, O_E_BGANNregerr, O_E_BGANNannerr], 2)
    for i in range(27):
        print(types[i])
        if (i+1)%3 == 1:
            savefile1.write("{A0} \t&\t {A1} \t&\t {A2} \t&\t ${B}\\pm{Berr}$ \t&\t ${C}$ \t&\t ${D}$\\\\ \n".format(A0=rowcatgs[i], A1=types[i], A2=R[i], 
                                        B=aperdata[i][0], Berr=aperdata_err[i][0], 
                                        C="---", D="---"))
        else:
            savefile1.write("{A0} \t&\t {A1} \t&\t {A2} \t&\t ${B}\\pm{Berr}$ \t&\t ${C}\\pm{Cerr}$ \t&\t ${D}$\\\\ \n".format(A0=rowcatgs[i], A1=types[i], A2=R[i], 
                                        B=aperdata[i][0], Berr=aperdata_err[i][0], 
    ...
    # O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad. EXPECTED
    savefile1.write("\n\n\n\n\n")   
    savefile1.write("O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad. --> star aper, sky aper, sky annul EXPECTED\n")  
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{Aper Type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]}\\\\ \n")
    savefile1.write("\\hline \hline \n")
    rowcatgs = ["O - E - $\\Delta \\text{bg}_{\\text{aper}}$ - grad.", "",
                "O - E - $\\Delta \\text{bg}_{\\text{annul}}$ - grad.", ""]
    types = np.tile(["star aper", "sky aper"], 2)
    R = np.tile([starpar[3], 15], 2)
    aperdata = np.round([newres_regEXPECT, bckgregOPT_regEXPECT,
                         newres_anEXPECT, bckgregOPT_anEXPECT], 2)

    for i in range(4):
        print(types[i])
        savefile1.write("{A0} \t&\t {A1} \t&\t {A2} \t&\t ${B}$\\\\ \n".format(A0=rowcatgs[i], A1=types[i], A2=R[i], B=aperdata[i]))
        if (i+1)%2 == 0:
            savefile1.write("\\hline \n")        



    # O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad.
    savefile1.write("\n\n\n\n\n")   
    savefile1.write("O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad. --> star aper, sky aper, sky annul\n")  
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{Aper Type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \hline \n")
    rowcatgs = ["O - E - grad.", "",
                "O - E - $\\Delta \\text{bg}_{\\text{aper}}$ - grad.", "",
                "O - E - $\\Delta \\text{bg}_{\\text{annul}}$ - grad.", ""]
    types = np.tile(["star aper", "sky aper"], 3)
    R = np.tile([starpar[3], 15], 3)
    aperdata = np.round([oldres, bckgregOPT_old,
                         newres_reg, bckgregOPT_reg,
                         newres_an, bckgregOPT_an], 2)
    aperdata_err = np.round([oldres_err, bckgregOPT_old_err,
                             newres_reg_err, bckgregOPT_reg_err,
                             newres_an_err, bckgregOPT_an_err], 2)
                             
    for i in range(6):
        print(types[i])
        savefile1.write("{A0} \t&\t {A1} \t&\t {A2} \t&\t ${B}\\pm{Berr}$ \t&\t ${C}\\pm{Cerr}$ \t&\t ${D}$\\\\ \n".format(A0=rowcatgs[i], A1=types[i], A2=R[i], B=aperdata[i][0], Berr=aperdata_err[i][0], 
                                    C=aperdata[i][1], Cerr=aperdata_err[i][1], D=aperdata[i][2]))
        if (i+1)%2 == 0:
            savefile1.write("\\hline \n")    
    ...
    # O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad. ABSOLUTE 
    savefile1.write("\n\n\n\n\n")   
    savefile1.write("O-E - grad., O-E - DeltaBG_[aper] - grad., O-E - DeltaBG_[annul] - grad. --> star aper, sky aper, sky annul ABSOLUTE\n")  
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{Aper Type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \hline \n")
    rowcatgs = ["O - E - grad.", "",
                "O - E - $\\Delta \\text{bg}_{\\text{aper}}$ - grad.", "",
    ...
    types = np.tile(["star aper", "sky aper"], 3)
    R = np.tile([starpar[3], 15], 3)
    aperdata = np.round([oldresABS, bckgregOPT_oldABS,
                         newres_regABS, bckgregOPT_regABS,
                         newres_anABS, bckgregOPT_anABS], 2)
    aperdata_err = np.round([oldresABS_err, bckgregOPT_oldABS_err,
                             newres_regABS_err, bckgregOPT_regABS_err,
                             newres_anABS_err, bckgregOPT_anABS_err], 2)
    ...
    savefile1.write("\\textbf{Image Type} \t&\t \\textbf{R [pixels]} \t&\t \\textbf{$\\Sigma$ [ADU]} \t&\t \\textbf{$\\mu$ [ADU]} \t&\t \\textbf{MD [ADU]} \\\\ \n")
    savefile1.write("\\hline \n")   
    rowcatgs = ["O-E-grad.", "", "",
                "\\textbf{O-$\\text{bg}_{\\text{aper}}-grad.}$", "", "",
                "\\textbf{O-$\\text{bg}_{\\text{annul}}-grad.}$", "", ""]
    types = []
